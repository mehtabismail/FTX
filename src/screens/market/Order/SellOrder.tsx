...
const SellOrder = (props: any) => {
  ...
  //   useActiveSellOrderQuery<any>({baseCurrency, quoteCurrency});

  const updatingData = async () => {
    const response = await fetch(
      `${
        navigationStrings.BASE_URL
      }order/sell?base_currency=${baseCurrency.toLowerCase()}&&quote_currency=${quoteCurrency.toLowerCase()}`,
      {
        method: 'GET',
        headers: {
